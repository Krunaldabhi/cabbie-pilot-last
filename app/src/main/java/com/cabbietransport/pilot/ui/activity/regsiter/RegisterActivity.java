...
import com.cabbietransport.pilot.data.network.model.ServiceType;
...
import com.cabbietransport.pilot.ui.countrypicker.Country;
import com.cabbietransport.pilot.ui.countrypicker.CountryPicker;
...
import com.cabbietransport.pilot.ui.activity.main.MainActivity;
...
public class RegisterActivity extends BaseActivity implements RegisterIView {
    ...
    @BindView(R.id.vehicleRegionSpinner)
    AppCompatSpinner vehicleRegionSpinner;
    @BindView(R.id.txtVehicleCityNumber)
    EditText txtVehicleCityNumber;
    ...
    @BindView(R.id.limoFleetOwner)
    AppCompatSpinner spinnerLimoOwner;
    ...
    private int selected_limo_owner = -1;
    private List<ServiceType> lstServiceTypes = new ArrayList<>();
    private List<LimoOwnerResponse> lstLimoOwners = new ArrayList<>();
    ...
    public void initView() {
        ...
        setupSpinner(null);
        setUpLimoOwnerSpinner(new ArrayList<LimoOwnerResponse>());
        presenter.getSettings();
        presenter.getLimoOwners();
        ...
        spinnerLimoOwner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                selected_limo_owner = i;
            }
        ...
        if (BuildConfig.DEBUG) {
            ...
            vehicleRegionSpinner.setSelection(0);
            txtVehicleCityNumber.setText("");
    ...
    private boolean validation() {
        if (txtEmail.getText().toString().isEmpty()) {
            ...
            return false;
        } else if (txtVehicleCityNumber.getText().toString().trim().isEmpty()) { // TODO
            Toasty.error(this, getString(R.string.invalid_car_number), Toast.LENGTH_SHORT, true).show();
            return false;
        } else if (txtVehicleNumber.getText().toString().trim().isEmpty()) { // TODO
            Toasty.error(this, getString(R.string.invalid_car_number), Toast.LENGTH_SHORT, true).show();
            return false;
    ...
    private void register(String countryCode, String phoneNumber) {
        ...
        map.put("fleet", toRequestBody(lstLimoOwners.get(selected_pos).getId() + ""));
        map.put("service_model", toRequestBody(txtVehicleModel.getText().toString()));
        map.put("service_number", toRequestBody(vehicleRegionSpinner.getSelectedItem().toString() + " " + txtVehicleCityNumber.getText().toString() + " " + txtVehicleNumber.getText().toString()));
...
//                    fbPhoneLogin(countryCode, countryDialCode, phoneNumber.getText().toString());
    ...
    @Override
    public void onSuccess(List<LimoOwnerResponse> response) {
        lstLimoOwners = response;
        setUpLimoOwnerSpinner(response);
    }
    ...
    private void setUpLimoOwnerSpinner(List<LimoOwnerResponse> responses) {
        ArrayList<String> lstLimoOwners = new ArrayList<>();
        for (LimoOwnerResponse response : responses) {
            lstLimoOwners.add(response.getCompany());
        }

        ArrayAdapter<String> mLimoOwnerAdapter = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, lstLimoOwners);
        mLimoOwnerAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        spinnerLimoOwner.setAdapter(mLimoOwnerAdapter);
    }
